...
documents = fields.map(lambda x: x[3].split(" ")) #Contains lists of words appearing
# in each document. RDD data type

# Store the document names for later:
documentNames = fields.map(lambda x: x[1]) # document name stored in field 1
...
gettysburgTF = hashingTF.transform(["illumination"])
